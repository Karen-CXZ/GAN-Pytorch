...
def train():
    ...
    for epoch in range(opt.epochs):
        for i, (imgs, _) in enumerate(data_loader):
            ...
            d_loss = -torch.mean(discriminator(real_imgs))+torch.mean(discriminator(gen_imgs.detach())) + opt.lameda_gp * gradient_penalty
            ...
            if batches_done % opt.sample_interval == 0:
                save_image(gen_imgs.data[:25], "image/%d.png" % batches_done, nrow=5, normalize=True)
